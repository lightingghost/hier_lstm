...
class BucketIter(mx.io.DataIter):
    def __init__(self, data, label, batch_size, init_states, 
        ...
        label_lens = np.argwhere(label == 1)[:, 1] + 1
        data_lens = np.argwhere(data == 1)[:, 1] + 1
        self.label_bucket_lens = default_gen_buckets(label, label_lens, batch_size)
        self.data_bucket_lens = default_gen_buckets(data, data_lens, batch_size)

        self.data_name = data_name
        self.label_name = label_name

        self.label_bucket_lens.sort()
        self.data_bucket_lens.sort()
        self.idx_buckets = [[] for _ in buckets]
